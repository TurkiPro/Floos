...
class StatisticsScreen extends StatelessWidget {
                ...
                builder: (context, contribSnap) {
                  ...
                  return StreamBuilder<List<RecurrenceRule>>(
                    ...
                    builder: (context, rulesSnap) {
                      ...
                      // The category insights describe DISCRETIONARY money only
                      // (rows not generated by a recurring rule): where the
                      // spending you actually control goes, and what to trim.
                      // Fixed obligations get their own line on the month card,
                      // so they don't dominate every ranking. Both this cycle and
                      // the norm behind the trends are filtered the same way, so a
                      // category carrying recurring subscriptions isn't judged
                      // against an inflated past.
                      final discretionaryRows = rows
                          .where((r) => r.txn.recurrenceId == null)
                          .toList();
                      // Per-category slices of this period's discretionary spend,
                      // for the top-categories card and its drill-downs.
                      final breakdown = categoryBreakdown(discretionaryRows
                      ...
                      final cuts = cutSuggestions(
                          ...
                          byId: byId,
                          trends: trends,
                          periodTotal: s.discretionaryThisMonth);
    ...
    return _card(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: AppSpacing.md),
          // Committed (fixed obligations) vs discretionary (what you chose) —
          // the split that makes the rest of the screen make sense.
          _committedSplit(context, s, money),
  ...
  /// The month's spend split into fixed obligations vs the money you actually
  /// control — a two-tone bar with legends and a one-line explanation.
  Widget _committedSplit(
      BuildContext context, StatisticsSummary s, NumberFormat money) {
    final scheme = Theme.of(context).colorScheme;
    final committed = s.committedThisMonth;
    final discretionary = s.discretionaryThisMonth;
    final total = committed + discretionary;
    final committedPct = total > 0 ? committed / total : 0.0;
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        ClipRRect(
          ...
          child: Row(
            children: [
              Expanded(
                flex: (committedPct * 1000).round().clamp(0, 1000),
                child: Container(height: 12, color: scheme.outline),
              ),
              Expanded(
                flex: ((1 - committedPct) * 1000).round().clamp(0, 1000),
                child: Container(height: 12, color: scheme.primary),
              ),
            ],
          ),
        ),
        ...
        Row(
          children: [
            _legend(context, 'التزامات ثابتة', '${money.format(committed)} ⃁',
                scheme.outline),
            const SizedBox(width: AppSpacing.lg),
            _legend(context, 'إنفاق متغيّر', '${money.format(discretionary)} ⃁',
                scheme.primary),
          ],
        ),
        const SizedBox(height: AppSpacing.xs),
        Text(
          'المتغيّر هو ما تتحكّم فيه فعليًا؛ الثابت التزامات متكرّرة '
          '(إيجار، اشتراكات، تحويلات).',
          style: TextStyle(
  ...
  Widget _essentialsCard(
    ...
    return _card(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          _label(context, 'إنفاقك المتغيّر: أساسيات مقابل كماليات'),
          const SizedBox(height: AppSpacing.xs),
          Text('لا يشمل التزاماتك الثابتة',
              style: TextStyle(
                  fontSize: AppTextSizes.label,
                  color: Theme.of(context).colorScheme.onSurfaceVariant)),
          const SizedBox(height: AppSpacing.md),
  ...
  Widget _topCategoriesCard(
    ...
    return _card(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          _label(context, 'أكثر الفئات إنفاقًا (المتغيّر هذا الشهر)'),
          const SizedBox(height: AppSpacing.xs),
          Text('من إنفاقك المتغيّر — اضغط فئة لعرض عملياتها',
              style: TextStyle(
                  fontSize: AppTextSizes.label,
                  color: scheme.onSurfaceVariant)),
          const SizedBox(height: AppSpacing.md),
